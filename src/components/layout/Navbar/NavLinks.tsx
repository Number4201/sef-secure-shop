...
const NavLinks: React.FC = () => {
  return (
    <nav className="hidden lg:flex items-center space-x-6">
      <Link to="/" className="text-white hover:text-esejfy-burgundy transition-colors font-medium">
        Domů
      </Link>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="link" className="text-white hover:text-esejfy-burgundy transition-colors flex items-center gap-1 p-0 font-medium">
            Produkty <ChevronDown size={14} />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="center" className="w-56 bg-esejfy-dark-secondary text-white border-esejfy-dark-accent">
          {categories.map((category) => (
            <DropdownMenuItem key={category.path} asChild className="text-white hover:bg-esejfy-dark-accent focus:bg-esejfy-dark-accent">
      ...
      <Link to="/o-nas" className="text-white hover:text-esejfy-burgundy transition-colors font-medium">
        O nás
      </Link>
      <Link to="/kontakt" className="text-white hover:text-esejfy-burgundy transition-colors font-medium">
        Kontakt
      </Link>
    </nav>
  );
